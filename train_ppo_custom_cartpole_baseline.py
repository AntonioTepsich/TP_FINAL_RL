...
from ppo import PPODiagnostic  # tu clase
...
def train_one_seed(seed: int):
    ...
    # ======== LOOP DE TRAINING (TUYO, CASI IGUAL) ========
    for update in tqdm(range(n_updates), desc=f"Entrenando PPO custom (seed={seed})", unit="update"):
        ...
        # Si el rollout termina en medio de un episodio, guardamos ese reward parcial
        if total_reward_ep > 0:
            episode_rewards.append(total_reward_ep)
            total_reward_ep = 0.0

        # ======== GAE + RETURNS (igual que el tuyo) ========
    ...
    # ======== GUARDAR CURVAS DE TRAINING (OPCIONAL, POR SEED) ========
    np.savez(
    ...
    eval_env.reset(seed=seed + 1000)  # para no mezclar con training
    ...
    with torch.no_grad():
        for _ in range(N_EVAL_EPISODES):
            ...
            while not done:
                ...
                # Política "determinística": tomamos la acción greedy
...
def main():
    ...
    all_rewards = np.stack(all_rewards, axis=0)  # [N_SEEDS, N_EVAL_EPISODES]
